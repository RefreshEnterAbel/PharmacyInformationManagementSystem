<template>
  <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
    <div class="flex justify-center items-center">
      ...
      <span class="text-gray-700 font-semibold text-2xl">{{ $t('reset_password') }}</span>
    </div>
    <form class="mt-4" @submit.prevent="reset" @keydown="form.onKeydown($event)">
      ...
      <div class="mb-3">
        ...
        <label class="block">
          ...
          <input v-model="form.email" :class="{ 'is-invalid': form.errors.has('email') }" type="email" name="email"
                 class=" form-input mt-1 block w-full border-2 rounded-md focus:border-indigo-600"
          >
        ...
        <label class="block mt-3">
          <span class="text-gray-700 text-sm">{{ $t('confirm_password') }}</span>
          <input v-model="form.password" :class="{ 'is-invalid': form.errors.has('password') }" type="password"
                 name="password" class=" form-input mt-1 block w-full border-2 rounded-md focus:border-indigo-600"
          >
